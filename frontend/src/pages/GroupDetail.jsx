...
export default function GroupDetail() {
  ...
  const { user } = useAuth();
  const { data: group, loading: groupLoading } = useFetch(`/api/groups/${groupId}`);
  ...
  // When group loads, initialise splits evenly
  const initSplits = (members, amount, currency) => {
  ...
  const handleFormChange = (field, value) => {
    setForm((f) => {
      ...
      if ((field === "amount" || field === "currency") && group) {
        updated.splits = initSplits(group.members, updated.amount || 0, updated.currency);
      }
      return updated;
    });
  };
  ...
  const openForm = () => {
    if (group) {
      setForm((f) => ({ ...f, splits: initSplits(group.members, f.amount || 0, f.currency) }));
    }
    setShowForm(true);
  };
  ...
  const submitTransaction = async (e) => {
    ...
    try {
      ...
      setForm({ description: "", amount: "", currency: "USD", date: today(), splits: initSplits(group.members, 0, "USD") });
      setShowForm(false);
      // Refresh balances
  ...
  return (
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "2rem 1rem" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
        ...
        <button onClick={showForm ? () => setShowForm(false) : openForm} style={primaryBtn}>
